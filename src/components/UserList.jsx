...
const UserList = () => {
  ...
  const fetchUsers = async () => {
    ...
    try {
      const data = await authFetch(
        "https://abundant-prosperity-production.up.railway.app/api/users"
      );
      setUsers(data);
    ...
    if (window.confirm("Are you sure you want to delete this user?")) {
      try {
        const response = await fetch(
          `https://abundant-prosperity-production.up.railway.app/api/users/${id}`,
          {
            method: "DELETE",
            headers: {
  ...
  const shouldShowActions = (role) => {
    // Only show actions for VIEWER users
    return role === "VIEWER";
  };
        ...
        <>
          ...
          <div className="px-6 py-4 border-t border-gray-200 flex items-center justify-between">
            <div className="text-sm text-gray-500">
              Showing <span className="font-medium">{sortedUsers.length}</span>{" "}
